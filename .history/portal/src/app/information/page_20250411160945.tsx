...
export default function InformationPage() {
      ...
      try {
        ...
        if (data && data.data) {
          if (Array.isArray(data.data)) {
            ...
            userInfo = data.data.find(user => user.email === tokenInfo.email);
            
            if (!userInfo) {
              // Thử tìm theo userId nếu không tìm thấy theo email
              userInfo = data.data.find(user => 
        ...
        if (userInfo) {
          setUserData({
            ...
            phoneNumber: userInfo.phone || "", // API uses 'phone' but we store as 'phoneNumber' internally
  ...
  const handleUpdateInfo = async (updatedData: { name: string; phoneNumber: string }) => {
    try {
      ...
      const response = await fetch(`http://localhost:3002/edit-infors?userId=${userId}`, {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          ...
          phone: updatedData.phoneNumber, // Convert our 'phoneNumber' to API's 'phone'
      ...
      if (updatedResponse.ok) {
        ...
        if (updatedData && updatedData.data) {
          if (Array.isArray(updatedData.data)) {
            updatedUserInfo = updatedData.data.find(user => 
        ...
        if (updatedUserInfo) {
          setUserData(prev => ({
            ...
            phoneNumber: updatedUserInfo.phone || prev.phoneNumber // API returns 'phone', store as 'phoneNumber'
          }));
        } else {
